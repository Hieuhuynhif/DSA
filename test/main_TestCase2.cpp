...
* b3: chạy lệnh g++ -o main main_TestCase2.cpp ID_BTL2_DSA.cpp nếu không gì hiện ra thì không bị lỗi code
...
void run_task_2(string fileName)
{
    ...
    file >> s;
    OUTPUT << s << endl;
    toRow = stoi(s.substr(6));
    file >> s;
    OUTPUT << s << endl;
    fromRow = stoi(s.substr(8));
    file >> s;
    OUTPUT << s << endl;
    toColFeature = stoi(s.substr(13));
    file >> s;
    OUTPUT << s << endl;
    fromColFeature = stoi(s.substr(15));
    file >> s;
    OUTPUT << s << endl;
    K_knn = stoi(s.substr(6));
    file >> s;
    OUTPUT << s << endl;
    test_size = stod(s.substr(10));
    OUTPUT << "------------------------------------" << endl;
